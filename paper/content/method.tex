...
12-lead ECG data was first obtained from the Physikalisch-Technische Bundesanstalt (PTB) Diagnostic ECG Database \citep{physionet}.
...
The exact details of this matrix varies by implementation, but this paper follows the implementation described by Daponte et al. \citep{daponte2021}.
Here, the authors propose that \(\Phi\) be composed of a combination of dominant leads.
Although less practical, this faired notably better than alternate methods, such an iid Gaussian sensing matrix \citep{mishra2012}.
...
Proper selection of the wavelet type is crucial in ensuring sparsity in the matrix \(C\) - early literature suggests biorthogonal wavelets to yield the bests results \citep{mishra2012}, but modern implementations tend to favor forms of a mexican hat wavelet \citep{daponte2021}.
Again, this paper follows the implementation described by Daponte et al., selecting the following basis matrix
\[ \Psi = [\Psi', u]\]
...
Given the above information, a sparse minimization problem can now be designed to solve for the coefficient matrix \(C\) with cost function \(J\) \citep{daponte2021}:
...
\newline
\indent \(\ell_1\)-norm (LASSO) \citep{yuan2006}:
\[ J = \left\| C \right\|_1 \]
\indent \(\ell_{2,1}\)-norm (Group LASSO) \citep{yuan2006}:
\[ J = \left\| C \right\|_2,1 = \sum_{n=1}^N \left\| c_{n\ \cdot} \right\|_2 \]
\indent \(\ell_{2,p}\)-norm (M-FOCUSS) \citep{cotter2005}:
